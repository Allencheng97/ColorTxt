...
- 复用主应用阅读能力：字体/行距/主题/语音/全屏/定时滚屏等；**阅读 / 编辑 / 语音朗读**与主界面共用 `colorTxt.ui.settings`（各窗内存独立，打开窗口时加载最后保存值，不做运行时互相同步）。朗读过滤 / 自动暂停见 [语音朗读](./语音朗读.md) → **「朗读设置」**。
...
- 底栏（`FindBookReaderFooter`）：左侧可挂番茄时钟（与主界面同款，设置在「阅读」页；关闭阅读器时自动停止）；自动暂停已开且正在播放时，「阅读进度」左侧同样显示 **`语音朗读：…`**（章节模式按**文档 / 自动续章**计，不是主窗侧栏目录）。右侧展示 `阅读进度：{章进度%}（{正序章号}/{总章数}，{当前行}/{总行}）` 与 `当前章节字数`（去空白后 `formatCharCount`）；章进度按阅读正序下标相对总章数计算（与主界面按全文行/滚动进度不同）。全屏时与章节导航一并收入底栏浮层（贴底感应显隐，同主界面）。
- **边界滚动切章**（**`findBookChapterAdvanceEnabled`**，默认 **开启**，写入共用 **`colorTxt.ui.settings`**）：找书阅读器专用；主界面「设置 → 阅读」**不展示**（`SettingsReadingPanel` 的 **`showFindBookChapterAdvanceOption`**）。入口：找书 **设置 → 阅读 → 边界滚动切章**。
  ...
  - **输入**：滚轮（阅读区 `@wheel.capture`，累计阈值防误触，只认正文区域）；空格 / `PageDown` / `PageUp` / 方向键（「到边界后再操作一次」）；**点击模式**下阅读区左/右键翻页。空格走 `ReaderMain.interceptReadonlySpacePageDown`；翻页快捷键走 `useFindBookReaderShortcuts.tryAdvanceChapterOnScroll`；点击翻页走 `ReaderMain.interceptReadonlyPageStep`。不要用 Monaco `onDidScrollChange` 检测超额滚动：到底后 `scrollTop` 不再变化。
