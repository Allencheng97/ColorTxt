...
| 找书设置相关 localStorage | `colortxt.findBook.settings`：缓存/下载目录、网络代理、找书阅读器侧栏等；**阅读 / 编辑 / 语音朗读 / 词典 / 边界滚动切章**与主界面共用 `colorTxt.ui.settings` |
...
**排错建议**：以 legado-E 源码 + Legado_Max 帮助 md 为准；社区教程（喵公子、破冰）作规则语义与样例补充；用具体
...
| 详情 / 阅读器 | `BookDetailPanel.vue`、`FindBookReaderPanel.vue`（编排，含边界滚动切章）、`FindBookReaderChapterSidebar.vue`、`useFindBookChapterSession.ts`、`constants/findBookChapterAdvance.ts` |
