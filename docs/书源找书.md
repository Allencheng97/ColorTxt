...
- **编辑**：分标签表单（基本 / 搜索 / 发现 / 详情 / 目录 / 正文…）；阅读器顶栏「编辑书源」默认打开「正文」。  
  ...
  - **全屏单字段 Monaco**（标签右侧「源码」图标，仅高亮候选字段）：`BookSourceFieldMonacoModal`
    ...
    `sourceRegex`/`replaceRegex`→`plaintext`；**其余→`javascript`（仅着色）**——并关闭 JS/TS
    诊断与 hover，避免规则 DSL（`{{ }}` / `$..` / `##`）红波浪线误报。  
...
| 单字段全屏 Monaco | `BookSourceFieldMonacoModal.vue`、`formatBookSourceFieldText.ts`（`js-beautify`） |
