...
- **边界滚动切章**（**`findBookChapterAdvanceEnabled`**，默认 **开启**，写入共用 **`colorTxt.ui.settings`**）：找书阅读器专用；主界面「设置 → 阅读」**不展示**（`SettingsReadingPanel` 的 **`showFindBookChapterAdvanceOption`**）。入口：找书 **设置 → 阅读 → 边界滚动切章**。
  ...
  - **输入**：滚轮（阅读区 `@wheel.capture`，累计阈值防误触，只认正文区域）；空格 / `PageDown` / `PageUp` / 方向键（「到边界后再操作一次」）。空格走 `ReaderMain.interceptReadonlySpacePageDown`；翻页快捷键走 `useFindBookReaderShortcuts.tryAdvanceChapterOnScroll`。不要用 Monaco `onDidScrollChange` 检测超额滚动：到底后 `scrollTop` 不再变化。
