...
└── shared/
    ...
    ├── voiceReadEngineConfig.ts  # 各引擎 API 密钥与模型字段（通义 / MiniMax / MiMo / 火山分开）
    ...
    ├── voiceReadEmotion.ts       # 情绪参数（通义 instruct / MiMo / 火山自然语言；MiniMax 枚举）
...
- **`voiceRead/`**：**`providerRegistry.ts`** 统一注册 TTS Provider，**`registerVoiceReadIpc.ts`** 暴露 `synthesize` / `listVoices` / `healthCheck`。火山引擎实现为 **`providers/volcengineProvider.ts`**（engine id **`volcengine`**），接入**豆包语音合成大模型 2.0**单向流式接口，以新版 **`X-Api-Key`** + **`X-Api-Resource-Id: seed-tts-2.0`** 鉴权并返回 **48 kHz PCM**；静态 **`voiceReadVolcengineVoices.ts`** 收录官方 2.0 主表 293 + 多语种表 151，共 **444 个音色**（中文、英文及其他多语种）。火山连接测试会合成一个汉字，产生极少量用量。
