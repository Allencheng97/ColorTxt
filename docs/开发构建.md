...
- **`useReaderClickModeAltHold.ts`**：按住 **Alt** 临时反转可选/点击模式（不写盘）；失焦 / 模态层 / 编辑模式清除。左键已按下后再按 Alt 本轮不切模式。主窗与找书阅读器共用。从点击模式临时切到可选时，`ReaderMain` 在捕获阶段抹掉鼠标/指针事件的 `altKey`，避免 Monaco 列选。
