...
│       │   └── voiceRead/              # 朗读分段、合成客户端、排播、音色解析、预览与缓存（含角色语种/方言）
│       ├── stores/
...
- **语音朗读**：**`useAppVoiceRead`** 驱动顶栏 **`VoiceReadToolbar`**；播放中 **`isVoiceReadNavigationBlocked`** 拦截侧栏 tab 切换与查找栏打开；朗读中空格暂停/播放、左右换行，**`shortcutService`** 仍吞掉行滚/页滚/章节/查找（字号等仍可用）；设置 **`voiceRead.*`** 持久化于 **`colorTxt.ui.settings`**（见 [语音朗读](./语音朗读.md)）；与 **定时滚动** 互斥（见 **「定时滚动」**）。
...
- **`useAppVoiceRead.ts`**：顶栏朗读主循环：行跳转、合成状态、退出清理；播放中 **`isVoiceReadNavigationBlocked`**。火山引擎走 PCM 排播；合成请求经 **`voiceReadSynthesisClient`** 带上槽位或角色语种/方言。见 [语音朗读](./语音朗读.md)。
