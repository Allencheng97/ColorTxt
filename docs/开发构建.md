...
│       │   ├── cjkWrapOptimize.ts        # 中文换行优化开关（全角标点 + break-all）
│       │   ├── cjkWrapStrings.ts         # Vite 包装 Monaco strings.isFullWidthCharacter
│       │   ├── lineSpacing.ts            # 物理行后常数行间距（get/set + ViewModel bridge）
│       │   # electron.vite：CJK canBreak/computeCharWidth/全角样字「汉」；LinesLayout 行间距；optimizeDeps.exclude monaco-editor
...
- **`appUi.ts`**：UI 常量：存储 key、侧栏宽度、字号/行高/行间距上下限与步进、`default*` 出厂默认等（无本地设置或与 `persistKey` 字段缺失时；见下文「阅读器字号与行高」「界面与阅读偏好默认值」）；re-export `readerPalette` 的 `applyReaderSurfaceToDocument` 等。
...
| `SettingsReadingPanel.vue`                           | 「阅读」：字号/行高滑块、压缩空行保留一行、引号/括号跨行匹配、**启用粘性章节标题**、**启用中文换行优化**（`monacoCjkWrapOptimize`）、**滚动**（平滑滚动、**滚动倍率** `mouseWheelScrollSensitivity`、**滚动加速倍率** `fastScrollSensitivity` / 按住 Alt）、全屏正文区宽度、**定时滚动**（范围 **`RadioGroup`**：一屏/一行；间隔 **`NumericInput`** 毫秒）。<br>（`monacoCustomHighlight` 来自 props，用于禁用跨行开关提示）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
