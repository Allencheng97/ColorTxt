...
- **`selectionToolbar.ts`**：选区工具条可选按钮 **`SelectionToolbarButtons`**（`copy` / `find` / `askAi`）、默认值（复制与问 AI 开、查找关）与 **`mergeSelectionToolbarButtons`**；持久化键 **`selectionToolbarButtons`**。
