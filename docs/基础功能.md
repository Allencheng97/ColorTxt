...
- **章节标题前后保留空行（`chapterTitleBlankMode`）**：默认 `before1`（前面 1 空行）。仅在开启「压缩空行」时生效；可选 `before1After1`（前 1 后 1）、`before2After1`（前 2 后 1）。若开了「保留一个空行」，非标题正文行下方仍会保留一行空行。
