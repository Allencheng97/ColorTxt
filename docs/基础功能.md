...
- **更多菜单**：侧栏标题行「更多」→ **移除失效文件**：对列表全部路径 `file:stat`，`!isFile`（含 ENOENT 不抛错）视为失效，经 `removeFileList` 移出侧栏列表（不删盘、不清阅读数据）；无失效时 toast「没有失效文件」。
- **拖放**：见 **`useAppWindowBindings.ts`**：侧栏列表区域合并进文件列表；**非侧栏**区域打开拖入列表中第一个支持的文件（`isSupportedBookPath` 或 **`.ctz` / `.ctzx` 彩读书包**，后者走 `openFilePath` → 书包导入，同「打开文件」）。
...
- **彩读书包默认密码**：设置项 **`bookPackPassword`**（「设置 → 常规 → 书包」→「书包默认密码」）；**默认空串**表示不设密码，导出 **`.ctz`**。非空时导出 **`.ctzx`**（**CTZE** 整包加密，AES-256-GCM + PBKDF2，见 `readerBookPackCrypto.ts`）。导入加密包时先尝试该密码；缺失或错误则弹出密码输入框。明文 `.ctz` 仍可直接导入（忽略该密码）。
...
- **入口**：顶栏「打开文件」（默认「电子书」过滤器含 `ctz`/`ctzx`；另有单独「彩读书包」过滤器）、非侧栏拖放、或系统关联双击 `.ctz`/`.ctzx`。**`parseReaderBookPackZip`**（含密码解密）校验 manifest / `kind`，合法才导入，否则报错中止（不走普通打开）。
- 加密包（通常为 `.ctzx`，亦可能误标为 `.ctz`）优先使用设置中的 **`bookPackPassword`**；若未设置或密码错误，弹出密码输入框重试（取消则中止导入；底栏「显示密码」勾选写入 `colorTxt.ui.bookPackPromptShowPassword`）。明文包忽略该密码。
