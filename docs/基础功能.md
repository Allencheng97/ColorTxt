...
- **查找**：主工具条「记笔记」后、「问 AI」前（需设置开启）；以当前选区/标注文案调用 **`ReaderMain.openFindWithSearchString`** 打开 Monaco 查找栏，并**关闭**工具条。
...
- **工具条显示项**（**设置 → 阅读 → 工具条**）：`selectionToolbarButtons`（`copy` / `find` / `askAi`）控制选区工具条是否显示「复制」「查找」「问 AI」；默认复制与问 AI 开、查找关。设置页以工具条预览 + 左上角复选框配置（`SettingsSelectionToolbarPreview.vue`）。
