...
- **正文右键**（`AppContextMenu`，Monaco `contextmenu: false`）：**只读**为 **复制**（无选区时禁用）/ **全选** / **编辑选中文本**（`icons.edit`，无选区禁用；选区物理原文超过 `maxPartialEditSelectionChars`（1000）时 **`appAlert`** 提示过大；`textarea` 弹窗改写源文件物理片段并写盘刷新展示；无物理行映射时提示改用顶栏进入编辑模式；默认快捷键 **`Ctrl+E` / `Command+E`**）；**编辑态**为剪切 / 复制 / 粘贴 / **全选**（及可用时的 **AI 智能排版：选中文本**）。智能排版 Diff 预览期另有侧栏菜单，不走此项。进入/退出编辑模式默认快捷键 **`Ctrl+/` / `Command+/`**（`toggleReaderEdit`）。
...
- 主题、侧栏展开、语法着色；**压缩空行** / **保留一个空行** / **行首缩进**；**`textConvertZh` / `textConvertLetter` / `textConvertDigit`**（顶栏「转换」，默认 **`off`**）；**`timedScroll`**（定时滚动范围与间隔，见 **`constants/timedScroll.ts`**）；**`readerEditShowLineNumbers`**、**`readerEditMinimap`**、**`editAutoRefreshChapterList`**；章节字数；字号与行高倍数、**`lineSpacingPx`**（行间距，默认 **0**）、**`letterSpacingPx`**（字间距，默认 **0**）；启动恢复会话；Monaco 高级换行；**`monacoCjkWrapOptimize`**（设置 → 阅读「启用中文换行优化」，默认 **开启**：简单换行下将 `“”`/`——`/`……` 等按全角估算，并近似 `word-break:break-all`；开启高级换行时运行时停用）；**滚动**（**`monacoSmoothScrolling`**、**`mouseWheelScrollSensitivity`** 默认 1、**`fastScrollSensitivity`** 默认 5 / 按住 Alt）；**`stickyChapterTitleEnabled`**（启用粘性章节标题，默认 **开启**）等。
