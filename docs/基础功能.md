...
配置项 label 为 **「数据管理」**，右侧 **「阅读数据」**（`icons.read`）+ **「清除缓存」**（`icons.clear`）；下方 hint 仍为「清除本地缓存数据（不影响界面相关的设置）。」。
...
- **为何需要 `sessionStorage` 标记**：
    ...
    - 实现：清存储前设置 **`skipUnloadPersistenceSessionKey`**，使 **`persistWindowUnloadState()`** 在卸载时**直接返回**；卸载流程里仍会 **`persistSettings()`**，只更新 `colorTxt.ui.settings`。
