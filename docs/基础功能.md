...
实现集中在 `src/renderer/src/constants/appUi.ts` 与 `src/renderer/src/monaco/readerEditorOptions.ts`（`readerEditorLineHeight`）；行间距另见 `src/renderer/src/monaco/lineSpacing.ts` 与 `electron.vite.config.ts` 对 Monaco `LinesLayout` 的 transform。
...
- **设置面板**：字号、行高、行间距均为滑块；行高滑块的上限随草稿字号变化；拖动字号若导致当前行高超限时，会自动下调行高草稿。
...
- 主题、侧栏展开、语法着色；**压缩空行** / **保留一个空行** / **行首缩进**；**`textConvertZh` / `textConvertLetter` / `textConvertDigit`**（顶栏「转换」，默认 **`off`**）；**`timedScroll`**（定时滚动范围与间隔，见 **`constants/timedScroll.ts`**）；**`readerEditShowLineNumbers`**、**`readerEditMinimap`**、**`editAutoRefreshChapterList`**；章节字数；字号与行高倍数、**`lineSpacingPx`**（行间距，默认 **0**）；启动恢复会话；Monaco 高级换行；**`monacoCjkWrapOptimize`**（设置 → 阅读「启用中文换行优化」，默认 **开启**：简单换行下将 `“”`/`——`/`……` 等按全角估算，并近似 `word-break:break-all`；开启高级换行时运行时停用）；**滚动**（**`monacoSmoothScrolling`**、**`mouseWheelScrollSensitivity`** 默认 1、**`fastScrollSensitivity`** 默认 5 / 按住 Alt）；**`stickyChapterTitleEnabled`**（启用粘性章节标题，默认 **开启**）等。
