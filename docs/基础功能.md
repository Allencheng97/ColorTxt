...
- **章节标题前后保留空行（`insertChapterTitleBlankLines`）**：默认关闭。仅在开启「压缩空行」时生效：关闭时合格章节标题前保留一个空行；启用后前两个空行、后一个空行。关闭时若开了「保留一个空行」，标题行下方仍会保留一行空行。
