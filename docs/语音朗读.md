...
| `volcengine` | 火山引擎 | **`volcengineProvider`** | **`engineConfig.volcengineApiKey`** | 48 kHz PCM | **豆包语音合成大模型 2.0**；新版 `X-Api-Key` 鉴权，请求固定携带 **`X-Api-Resource-Id: seed-tts-2.0`**；静态音色表 **`voiceReadVolcengineVoices.ts`** 收录官方 2.0 主表 293 个 + 多语种表 151 个，共 **444 个唯一音色**，含中文、英文和其他多语种 |

通义 / MiniMax / MiMo 设置页的 **`AppConnectionTestButton`** 仅校验密钥、**不合成**。火山引擎没有单独的无用量鉴权探测：连接测试会使用默认音色合成一个汉字，因此会产生极少量用量。
...
| **情绪标注** | AI 识别开启且引擎/模型支持时，可开 **`emotionEnabled`**（默认 true）；关闭则合成时不传情绪。通义 **`qwen3-tts-instruct-flash`** / MiMo / 火山引擎：AI 输出 **10～30 字自然语言** 语气描述 → **`mapEmotionForNaturalLanguageEngine`**（火山写入 `context_texts`）；MiniMax：固定 **9 枚举**标签，无法映射则不传 |
...
| **`utils/voiceReadVolcengineVoiceSelect.ts`** | 火山引擎 444 音色的语言分组、性别图标与说明 |
| **`main/voiceRead/providers/*.ts`** | 各云端 TTS Provider |
| **`shared/voiceReadVolcengineVoices.ts`** | 火山引擎豆包语音合成大模型 2.0 官方音色预设 |
