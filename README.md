...
| 豆包语音合成大模型 2.0 | 火山引擎 | 内置官方 **444 个音色**（中文、英文和其他多语种）；默认 24 kHz PCM（可改） |

火山引擎 TTS 使用控制台创建的新版 API Key，通过 `X-Api-Key` 与
`X-Api-Resource-Id: seed-tts-2.0` 鉴权。语速写入接口 `speech_rate`；默认 24 kHz PCM，可在设置中改采样率。官方无单独无用量鉴权接口，连接测试会合成一个汉字并产生极少量用量；
